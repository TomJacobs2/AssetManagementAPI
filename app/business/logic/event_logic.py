...
class EventLogic:
    ...
    def convert_request_to_create_schema(self, request_schema: EventRequest) -> EventCreate:
        ...
        event_type_id = get_type_by_code(db=self.session, model=EventTypeModel,
                                         code=request_schema.event_type_code).id
        event_subtype_id = get_type_by_code(db=self.session, model=EventSubTypeModel,
                                            code=request_schema.event_subtype_code).id
        raw_data = {"event_type_id": event_type_id, "event_subtype_id": event_subtype_id,
                    "geo_lat": request_schema.geo_lat, "geo_long": request_schema.geo_long,
                    "fk_subtype_link_id": asset_row.id,
                    "event_data": str({"data": request_schema.event_data, "asset": asset_row})}
